...
function Cards() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let updateData = {
                    ...
                    tournament_id: distributor
                }
                axios.post(`${apiBaseURL}/game/changeStatusCard`, updateData)
                    .then(function (response) {
                        if (response.data.status === 200) {
                            getPlayers();
                            Swal.fire(
    ...
    const columns = [
        ...
        { title: "Status", render: rowData => rowData.is_active === 1 ? 'Blocked' : "Active" }
    ];
        ...
        <div className="card card-outline card-info"  >
            <MaterialTable
                ...
                actions={[
                    (rowData) => {
                        return {
                            icon: rowData.is_active === 1 ? "block" : "settingsremote",
                            tooltip: rowData.is_active === 1 ? "Blocked" : "Active",
                            onClick: (event, row) => changeStatus(row.tournament_id, row.is_active, `${row.is_active === 0 ? "block" : "unblock"}`),
                        };
                    },
